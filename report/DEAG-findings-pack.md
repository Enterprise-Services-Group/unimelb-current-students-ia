...
### Finding 11 — The fragmentation runs the full length of the lifecycle, not just current students
A cross-corpus join of all four estates — prospective (`study.unimelb`, 300 pages), current (faculty + `students.unimelb`), alumni (`www.unimelb/alumni`, 116) and the full faculty domains — finds the same fragmentation at every lifecycle stage, not only the current one. **Careers is authored at ~10 independent locations across four domains** (prospective, central, 8 faculties, alumni), and faculty student-mentoring is never linked to the alumni mentoring students graduate into. **Prospective is as fragmented as current** — 300 central pages *plus* ~210 faculty `/study/` pages — so the apply→enrol handoff crosses a domain boundary with no shared vocabulary. **Alumni is largely current-student services re-hosted** on a fourth domain (careers, wellbeing, international), with no graduation→alumni bridge. And **research reputation is decoupled from research-student support** — MDHS hosts 63 research pages but only 1 HDR-candidature page. *Implication for DEAG:* a current-students-only mandate fixes one of three lifecycle handoffs; the seams students feel most — applying then enrolling, finishing then becoming alumni — sit *between* corpora and are the least owned. *(Evidence: `analysis/lifecycle-cross-corpus.md`.)*
